...
public final class BankGui {
    ...
    public static void open(Player player, EconomyService economyService, BankService bankService) {
        ...
        String bank = economyService.getCurrencySymbol()
                + String.format("%.2f", bankService.getBalance(player.getUniqueId()));
        String remaining = economyService.getCurrencySymbol()
                + String.format("%.2f", bankService.getRemainingLimit(player.getUniqueId()));

        inv.setItem(11, GuiUtils.item(Material.GOLD_INGOT, "Depositar 50", "Carteira: " + wallet,
                "Restante hoje: " + remaining));
        inv.setItem(12, GuiUtils.item(Material.GOLD_BLOCK, "Depositar 200", "Carteira: " + wallet,
                "Restante hoje: " + remaining));
        inv.setItem(13, GuiUtils.item(Material.DIAMOND_BLOCK, "§aDepositar Tudo", "§7Deposita o máximo possível",
                "§7respeitando o limite diário."));
